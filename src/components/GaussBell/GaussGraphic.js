...
export const getData = (xLimit) => {
	...
	const data = Array(numPoints)
		.fill(0)
		.map((_, i) => {
			const x = -xLimit + (i * 2 * xLimit) / (numPoints - 1);
			const y = normalDistribution(x, 0, variance);
			return { x, y };
		});
	return data;
};
...
export const LeftArea = (xLimit, alpha1) => {
	if (alpha1 === null) return [];
	if (alpha1 >= 0) return [];
	const data = getData(xLimit);
	return data.filter((datum) => datum.x >= -xLimit && datum.x <= alpha1);
};
export const RightArea = (xLimit, alpha2) => {
	if (alpha2 === null) return [];
	if (alpha2 <= 0) return [];
	const data = getData(xLimit);
	return data.filter((datum) => datum.x >= alpha2 && datum.x <= xLimit);
};
